...
abstract contract ImpermaxBaseStrategy is StrategyBase {
    ...
    uint256 internal constant _MIN_RATE_CHANGE = 100;
    /// @dev Threshold for liquidation process
    uint256 internal constant _MIN_PROFIT = 100;

    /// @notice Pool with rewards
    address public pool;
    /// @dev We will use it for calculate how much we can move to buybacks
    uint256 public lastPoolExchangeRate;

    event Decompound(address token, uint256 liquidated, uint256 result);
    ...
    function rewardPoolBalance() public view override returns (uint256) {
    ...
    function _redeem(uint256 amount) internal returns (uint256) {
        uint256 toRedeem = (amount * 1e18) / IBorrowable(pool).exchangeRate();
    ...
    function exitRewardPool() internal override {
        uint256 toRedeem = IERC20(pool).balanceOf(address(this));
        if (toRedeem > 1) {
            IERC20(pool).safeTransfer(pool, toRedeem);
    ...
    function _partiallyDecompound() internal {
        ...
        uint256 exchangeRateSnapshot = IBorrowable(pool).exchangeRate();
        uint256 profit = _expectedProfitAmount(exchangeRateSnapshot);
        if (profit == 0) {
            return;
        }

        // withdraw from pool to this contract
        uint256 toLiquidate = _redeem(profit);
        ...
        uint256 result = IFeeRewardForwarder(forwarder).liquidate(_underlyingToken, _rewardTokens[0], toLiquidate);
    ...
    function _expectedProfitAmount(uint256 currentRate) internal view returns (uint256) {
        if (lastPoolExchangeRate == 0) {
            return 0;
        }
        ...
        uint256 rateChange = currentRate - lastPoolExchangeRate;
        if (rateChange < _MIN_RATE_CHANGE) {
            // no actions if profit too low
            return 0;
        }
        uint256 profitAmount = (IERC20(pool).balanceOf(address(this)) * rateChange) / 1e18;
        uint256 profitAmountAdjusted = (profitAmount * _buyBackRatio) / _BUY_BACK_DENOMINATOR;
